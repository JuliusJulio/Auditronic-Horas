...
function updateSidebar() {
  ...
  if (!email) {
    if (db.auditores.length > 0) {
      ...
      document.getElementById('sidebar-email').textContent = a.rol;
    }
    return;
  }
  // Buscar auditor cuyo email coincida con el usuario logueado
  const me = db.auditores.find(a => (a.email || '').toLowerCase() === email.toLowerCase());
  if (me) {
    document.getElementById('sidebar-avatar').textContent = (me.iniciales || me.nombre.slice(0, 2)).toUpperCase();
    document.getElementById('sidebar-name').textContent = me.nombre;
    document.getElementById('sidebar-email').textContent = me.rol;
  } else {
    document.getElementById('sidebar-avatar').textContent = email.slice(0, 2).toUpperCase();
