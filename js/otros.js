...
function renderOtros() {
  // Leer valores de filtros antes de reconstruir selects
  const savedSoc = document.getElementById('otros-filtro-sociedad').value;
  const savedAud = document.getElementById('otros-filtro-auditor').value;

  // Llenar select de auditores
  updateAuditorSelect('otros-filtro-auditor');
  if (savedAud) document.getElementById('otros-filtro-auditor').value = savedAud;

  // Llenar select de sociedades con valores únicos existentes en otros
  const selSoc = document.getElementById('otros-filtro-sociedad');
  selSoc.innerHTML = '<option value="">Todas las sociedades</option>';
  const sociedadesUnicas = [...new Set(db.otros.map(o => o.sociedad).filter(s => s && s.trim()))].sort();
  sociedadesUnicas.forEach(s => {
    const o = document.createElement('option');
    o.value = s;
    o.textContent = s;
    selSoc.appendChild(o);
  });
  ...
  document.getElementById('otros-empty').style.display = filtrados.length ? 'none' : 'block';
  grid.innerHTML = '';
...
async function guardarOtro() {
  ...
  if (SB_LISTO) {
    if (editOtroId) { await SB.update('otros_trabajos', editOtroId, o); db.otros = db.otros.map(x => x.id === editOtroId ? o : x); }
    else { await SB.insert('otros_trabajos', o); db.otros.push(o); }
  } else {
