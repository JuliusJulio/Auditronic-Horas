  ...
  encs.forEach(e => {
    ...
    grid.innerHTML += `<div class="encargo-card ${tc}">
      ...
      <div class="encargo-actions">
        ${e.presupuesto > 0 ? `<button class="btn btn-ghost btn-sm" onclick="verDesglose('${e.id}')">▤ Cotejo</button>` : ''}
...
function verDesglose(encargoId) {
  ...
  document.getElementById('desglose-titulo').textContent = `Cotejo: ${e.nombre}`;
...
function abrirModalEncargo(id) {
  ...
  if (id) {
    ...
    ['enc-cliente', 'enc-codigo', 'enc-nombre', 'enc-presupuesto', 'enc-inicio', 'enc-fin', 'enc-notas'].forEach(i => document.getElementById(i).value = '');
...
async function guardarEncargo() {
  ...
  const enc = {
    id: editEncId || uid(), cliente, nombre,
  ...
  if (SB_LISTO) {
    if (editEncId) {
      await SB.update('encargos', editEncId, enc);
      db.encargos = db.encargos.map(e => e.id === editEncId ? enc : e);
    } else {
      await SB.insert('encargos', enc);
      db.encargos.push(enc);
    }
  } else {
